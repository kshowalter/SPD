...
//    //var input_section = state.inputs[section_name];
...
f.section_defined = function(system_id, section_name){
  ...
  if(Meteor.isServer){
    //active_system = state.status.active_system;
...
f.toggle_module = function(element){
  ...
  var c = element.getAttribute('module_ID').split(',')[1];
	...
	//console.log("state.webpage.selected_modules:", state.webpage.selected_modules);
	//console.log("state.webpage.selected_modules:", state.webpage.selected_modules);
  if( state.webpage.selected_modules[r][c] ){
    state.webpage.selected_modules[r][c] = false;
    //state.webpage.selected_modules_total--;
  } else {
    state.webpage.selected_modules[r][c] = true;
    //state.webpage.selected_modules_total++;
  }

  Meteor.call('select_module', state.webpage.selected_modules, function(err, returned){
...
//    (
//      settings.system.location.address  !== state.system_data.location.address ||
//      settings.system.location.city     !== state.system_data.location.city ||
//      settings.system.location.zip_code !== state.system_data.location.zip_code
//    )
//  );
//};
...
f.request_geocode = function(){
  if( settings.system.location.address &&
      settings.system.location.city &&
      settings.system.location.zip_code ){
    if( settings.system.location.address  !== state.system_data.location.address ||
        settings.system.location.city     !== state.system_data.location.city ||
        settings.system.location.zip_code !== state.system_data.location.zip_code ){
      state.system_data.location.new_address = true;
      state.system_data.location.address  = settings.system.location.address;
      state.system_data.location.city     = settings.system.location.city;
      state.system_data.location.zip_code = settings.system.location.zip_code;
...
f.set_coordinates = function(lat, lon){
  ...
  state.system_data.location.closest_station = wind_stations.get_closest(lat, lon);
