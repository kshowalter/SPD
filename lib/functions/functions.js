...
//f.section_defined = function(state, section_name){
//    //console.log("-"+section_name);
//    //var input_section = g.inputs[section_name];
//    //var output_section = g.system[section_name];
//    var output_section = state.user_input[section_name];
...
f.values_defined = function( state, value_obj ){
  var defined = true;
...
f.toggle_module = function(element){
  ...
  var c = element.getAttribute('module_ID').split(',')[1];
	...
	//console.log("g.webpage.selected_modules:", g.webpage.selected_modules);
...
//f.new_address(){
//  return (
//    g.system.location.address &&
//    g.system.location.city &&
//    g.system.location.zip_code &&
//    (
...
f.request_geocode = function(){
  if( g.system.location.address &&
      g.system.location.city &&
      g.system.location.zip_code ){
    if( state.system.location.address  !== state.system_data.location.address ||
        state.system.location.city     !== state.system_data.location.city ||
        state.system.location.zip_code !== state.system_data.location.zip_code ){
      g.system_data.location.new_address = true;
...
f.set_map_marker = function(){
  if( g.system_data.maps.marker_sat ){
    var latlng = L.latLng( g.system.location.lat, g.system.location.lon );
...
f.set_coordinates = function(lat, lon){
  ...
  if( g.system_data.location.new_address ){
    ...
    g.system_data.location.new_address = false;
  }
  //update();
  f.set_map_marker();
};
...
//  var checkMark = '<img src="./data/check.png" alt="Checked">';
//  var crossMark = '<img src="./data/cross.png" alt="Error">';
//  if( data === undefined && g.system.location.lat !== undefined ){ // loading last locations
...
f.set_coordinates_from_google_geocode = function(data){
  ...
  if( data.results[0].geometry.location !== undefined ){
    ...
    g.system_data.geocode.data = data;
    g.system_data.geocode.lat = lat;
    g.system_data.geocode.lon = lon;
