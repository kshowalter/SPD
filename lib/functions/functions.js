...
f.section_defined = function(section_name, user_id){
  ...
  if( typeof user_id === 'string' ){
    ...
    System_data.find({section_name:section_name}).forEach(function(doc){
      //console.log('    -', doc);
      if( ! doc.value ) {
        defined = false;
        return;
...
f.set_coordinates = function(lat, lon){
    g.perm.location.lat   = lat;
    g.perm.location.lon   = lon;
    g.system.location.lat = lat;
    g.system.location.lon = lon;
    g.system.location.closest_station = wind_stations.get_closest(lat, lon);
...
f.set_coordinates_from_geocode = function(data){
    ...
    if( data === undefined && g.perm.location.lat !== undefined ){ // loading last locations
        f.set_coordinates( g.perm.geocode.lat, g.perm.geocode.lng );
