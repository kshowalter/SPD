RAS127 = function(settings){
  var system = settings.system;
  var index;
  var slope = Number(system.roof.slope.split(':')[0]) / 12;
  var risk_category = settings.system.location.exposure_category;
  ...
  if( system.roof.mean_height <= 20 ){
    uplift_pressure_min = table[risk_category]["≤ 20'"][index];
  } else if( system.roof.mean_height <= 25 ){
    uplift_pressure_min = table[risk_category][">20' to ≤ 25'"][index];
  } else if( system.roof.mean_height <= 30 ){
    uplift_pressure_min = table[risk_category][">25' to ≤ 30'"][index];
  } else if( system.roof.mean_height <= 35 ){
    uplift_pressure_min = table[risk_category][">30' to ≤ 35'"][index];
  } else if( system.roof.mean_height <= 40 ){
    uplift_pressure_min = table[risk_category][">35' to ≤ 40'"][index];
  } else {
    console.log('roof height out of range');
  }

  console.log('uplift_pressure_min', uplift_pressure_min);
