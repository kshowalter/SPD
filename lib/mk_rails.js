...
rails = {
	...
	setModules: function()
	{
		//console.log(settings.webpage.selected_modules);
		rails.tiles = [];
		var r = 1;
		var c = 1;
		while(settings.webpage.selected_modules[r] !== undefined)
		{
			var row = [];
			while(settings.webpage.selected_modules[r][c] !== undefined)
			{
				//console.log("settings.webpage.selected_modules[%s][%s])", r, c, settings.webpage.selected_modules[r][c]);
				row.push({ r:r-1, c:c-1, value:settings.webpage.selected_modules[r][c], searched:false, rail:false });
				c++;
			}
		...
		//console.log("setTiles(settings.webpage.selected_modules):", settings.webpage.selected_modules);
