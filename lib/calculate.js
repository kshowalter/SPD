...
//  var f = settings.f;
  ...
  if( section_defined(active_system, 'array') && section_defined(active_system, 'module') ){
    ...
    system.array.isc = system.array.module.isc * system.array.num_strings;
    system.array.voc = system.array.module.voc * system.array.modules_per_string;
    system.array.imp = system.array.module.imp * system.array.num_strings;
  ...
  //inputs.inverter.loadcenter_type = settings.f.obj_names(inputs.inverter.loadcenter_types);
  if( system.inverter.loadcenter_types ) {
    var loadcenter_type = system.inverter.loadcenter_types;
    var AC_options = inputs.inverter.loadcenter_types[loadcenter_type];
    inputs.AC.type.options = AC_options;
    //data.opt.inverter.types[loadcenter_type];

    //inputs.AC['type'] = f.obj_names( settings.data.opt.inverter.type );
  }
  if( system.inverter.interconnection_voltage ) {
    system.inverter.conductors = settings.data.opt.inverter.conductors[system.inverter.interconnection_voltage];
    system.inverter.num_conductors = system.inverter.conductors.length;

  }
  if( section_defined(active_system,  'AC') ){

    system.inverter.wire_size = "-Undefined-";
  }
...
//////////////////////////////////////////////////////
  ...
  if( section_defined(active_system,  'module') ){
    ...
    var specs = PV_Components.findOne({ type:'modules', make:system.array.module_make, model:system.array.module_model });
    if(specs){
      system.array.module.pmp = specs.pmp;
      system.array.module.isc = specs.isc;
      system.array.module.voc = specs.voc;
      system.array.module.imp = specs.imp;
      system.array.module.vmp = specs.vmp;
      system.array.module.width = specs.width;
      system.array.module.length = specs.length;
    } else {
  ...
  settings.webpage.selected_modules.slice(1).forEach(function(row){
