...
calculate = function(settings){
  ...
  var active_system = settings.status.active_system;

  settings.notes = [];

  var inputs = settings.inputs;

  if( settings.system.location.exposure_category === 'B'){
    settings.notes.push('Category B selected, category C will be used in caclulations.');
    settings.system.location.exposure_category = 'C';
  }

  if(settings.system_data.location.closest_station){
    system.location.low_temp = settings.system_data.location.closest_station['Extreme min'];
    system.location.high_temp_max = settings.system_data.location.closest_station['High Temp 0.4%'];
    system.location.high_temp = settings.system_data.location.closest_station['High Temp 2% Avg.'];
  }
  ...
  if( system.array.pmp > 10000 ){
    settings.notes.push('System size exceeds 10kW');
    settings.status.inCompliance = false;
