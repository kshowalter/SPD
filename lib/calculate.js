...
calculate = function(state){
  var config_options = state.config_options;
  ...
  if( state.system.location.exposure_category === 'B'){
    state.notes.push('Category B selected, category C will be used in caclulations.');
    state.system.location.exposure_category = 'C';
  }
  ...
  if( system.array.pmp > 10000 ){
    state.notes.push('System size exceeds 10kW');
  ...
  if( system.inverter.grid_voltage ) {
    ...
    system.inverter.conductors = state.data.opt.inverter.conductors[grid_voltage];
  ...
  if( section_defined(active_system, 'roof') && state.system.location.exposure_category ){
    ...
    */
    ...
    system.roof.uplift_pressure_min = RAS127(state);
