mk_inputs = function(state){
  //console.log('I will now create the initial input state....');
  //state = state || {};
  ...
  state.user_input = f.add_sections(state.inputs);
  //console.log(state.user_input);
  ...
  //state.webpage.sections = Object.keys(state.inputs);
  ...
  //state.components.makes = PV_Components.find({}).map(function(doc){
  //  return doc.make;
  //});
  //state.components.models = PV_Components.find({}).map(function(doc){
  //  return doc.model;
  //});
  ...
  //state.inputs = state.inputs; // copy input reference with options to inputs
  //state.inputs = f.blank_copy(state.inputs); // make input section blank
  //state.system_formulas = state.system; // copy system reference to system_formulas
  state.system = f.blank_copy(state.inputs); // make system section blank
  state.system.array.module = {};

  //f.merge_objects( state.inputs, state.system );
