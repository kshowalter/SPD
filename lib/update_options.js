update_options = function( system_id, section_name, value_name){
  ...
  if(section_name === 'array' && value_name === 'module_make'){
    ...
    // grid_voltage
    var make = System_data.findOne({ system_id:system_id, section_name:section_name, value_name:'inverter_make' }).value;
    var model_id = System_data.findOne( { system_id:system_id, section_name:section_name, value_name:'inverter_model'} )._id;

    if( settings.system.inverter.loadcenter_type ){
      var voltages = settings.system.inverter.loadcenter_type.split('/').map(function(voltage_string){
