var model_from_make = function(system_id, section_name){
  var make = System_data.findOne({ system_id:system_id, section_name:section_name, value_name:'make' });
  new_options = _.uniq(
    Components.find({
      type: section_name+"s",
      make:make,
...
update_options = function( system_id, section_name, value_name){
  ...
  if(section_name === 'module' && value_name === 'make'){
    ...
    var loadcenter_type = System_data.findOne({ system_id:system_id, section_name:section_name, value_name:value_name });
    new_options = settings.in.opt.AC.loadcenter[loadcenter_type];
