mk_svg = f.mk_svg = function(drawing_parts, drawing_settings){
  ...
  //svg_document.setAttribute('x', 0);
  //svg_document.setAttribute('y', 0);
  //svg_document.setAttribute('width', drawing_size.w);
  //svg_document.setAttribute('height', drawing_size.h);
  ...
  function mk_svg_elem(item){
    ...
    /*
    if(Meteor.isServer){
      var file_path = path.resolve('.public/' + item.href);
      var bitmap = Assets.getBinary('../' + file_path);
      //var bitmap = fs.readFileSync(file_path);
      var base64_image = new Buffer(bitmap).toString('base64');
      item.href = 'data:image/png;base64,'+base64_image;
    }
    */
